...
function extractUserInfo(ctx) {
    ...
    if (ctx?.userId && ctx?.userToken) {
        userId = ctx.userId;
        token = ctx.userToken;
        console.log(`[create_timetable] 从ctx直接获取用户信息: ${userId}`);
        return { userId, token };
    }
    ...
    if (fullAgentId && fullAgentId.includes(':')) {
        ...
        console.log(`[create_timetable] 从agent.id提取userId: ${userId}`);
    }
    // 方法3: 从 ctx.agentId (sessionId) 查找userId映射
    if (!userId && ctx?.agentId) {
        userId = token_store_1.tokenStore.getUserBySession(ctx.agentId);
        console.log(`[create_timetable] 从session映射查找userId: ${userId} (sessionId: ${ctx.agentId})`);
    }
    // 获取用户Token
...
async function executeCreateTimetable(args, ctx) {
    console.log('[工具] 🚀 create_timetable():', JSON.stringify(args, null, 2));
    try {
        // 1. 提取用户信息
        const { userId, token } = extractUserInfo(ctx);
        if (!userId) {
            console.error('[create_timetable] ❌ 无法确定用户ID');
            return {
                ok: false,
        ...
        if (!token) {
            console.error('[create_timetable] ❌ 未找到用户Token');
            return {
                ok: false,
                error: `未找到用户 ${userId} 的认证Token`
            };
        }
        console.log(`[create_timetable] ✅ 使用用户 ${userId} 的Token`);
        ...
        console.log(`[create_timetable] 获取到 ${userProjects.length} 个项目用于匹配`);
        ...
        if (unmatchedTasks.length > 0) {
            console.error(`[create_timetable] ❌ 任务名称验证失败，以下任务不在用户项目列表中: ${unmatchedTasks.join(', ')}`);
            return {
                ok: false,
...
请使用上述项目名称重新填写时间表。`
        ...
        console.log(`[create_timetable] ✅ 任务验证通过: ${uniqueTaskNames.join(', ')}`);
        // 4.5. 验证是否提供了子任务信息
        ...
        if (missingSubtasks.length > 0) {
            console.error(`[create_timetable] ❌ 子任务验证失败，以下时间段缺少子任务信息:`);
...
注意：planned_task 需要对应 planned_subtask，actual_task 需要对应 actual_subtask`
        ...
        console.log(`[create_timetable] ✅ 子任务存在性验证通过`);
        ...
        console.log(`[create_timetable] 转换完成: ${apiRequest.time_slots.length} 个时间槽`);
        // 计算统计信息
        const stats = calculateStats(apiRequest, validDates);
        console.log(`[create_timetable] 统计: 项目匹配=${stats.matched_projects}, 子任务匹配=${stats.matched_subtasks}, 未匹配=${stats.unmatched_tasks}`);
        ...
        // 6. 调用后端 API 批量创建时间表
        const url = (0, config_1.getApiUrl)(`/api/v1/schedule/time-slots/batch?user_id=${numericUserId}`);
        console.log(`[create_timetable] 🌐 请求后端 API: ${url}`);
        const response = await axios_1.default.post(url, apiRequest, {
            headers: (0, config_1.getRequestHeaders)(token),
        });
        const result = response.data;
        console.log(`[create_timetable] ✅ 成功创建时间表: ${result.created_count} 个时间段`);
        // 7. 返回成功结果
        return {
            ok: true,
            data: {
                success: true,
                message: `成功创建 ${result.created_count} 个时间段，覆盖 ${stats.dates_covered.length} 个日期`,
                timetable: {
        ...
        console.error(`[create_timetable] ❌ 执行失败:`, error.message);
