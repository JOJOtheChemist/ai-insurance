...
router.post('/chat', auth_1.authenticateToken, async (req, res) => {
    ...
    if (sessionId && userId) {
        token_store_1.tokenStore.setSession(sessionId, userId);
    }
    if (!message) {
    ...
    // 检查是否有其他请求正在处理这个特定会话
    if (agent_service_1.agentManager.isProcessing(lockKey)) {
        console.log(`\n[排队] 会话 ${lockKey} 正在处理中，消息: ${message}`);
        return res.status(429).json({ error: '该会话正在处理中，请稍后再试' });
    }
    agent_service_1.agentManager.setProcessing(lockKey, true);
    ...
    try {
        ...
        const agent = await agent_service_1.agentManager.getOrCreateAgent(sessionAgentConfig);
        ...
        const toolHandler = (event) => {
            toolCount++;
            console.log(`[工具执行 ${toolCount}] ${event.call.name}: ${event.call.inputPreview} → ${JSON.stringify(event.call.result)}`);
            emitter.sendTool({
                index: toolCount,
                ...
                for await (const envelope of agent.subscribe(['progress'])) {
                    ...
                    switch (event.type) {
                        ...
                        case 'tool:error':
                            ...
                            break;
                        case 'done':
                            console.log(`[对话完成] 工具调用次数: ${toolCount}, 原因: ${event.reason}, bookmark: ${JSON.stringify(envelope.bookmark)}`);
                            ...
                            agent_service_1.agentManager.setProcessing(lockKey, false);
        ...
        console.log(`[发送前状态] state=${statusBefore.state}, breakpoint=${statusBefore.breakpoint}, step=${statusBefore.stepCount}, messages=${statusBefore.lastSfpIndex + 1}`);
        // 发送用户消息
        ...
        console.log(`[发送后状态] state=${statusAfter.state}, breakpoint=${statusAfter.breakpoint}, step=${statusAfter.stepCount}`);
        // 每2秒检查一次状态（调试用）
        statusCheck = setInterval(async () => {
            const status = await agent.status();
            console.log(`[状态] state=${status.state}, breakpoint=${status.breakpoint}, step=${status.stepCount}`);
        }, 2000);
        // 客户端断开时清理
        req.on('close', () => {
