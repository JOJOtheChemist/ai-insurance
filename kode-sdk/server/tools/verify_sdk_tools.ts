...
import { InsuranceSearch } from './insurance_search';
import { InsuranceFilter } from './insurance_filter';
import { InsuranceInspect } from './insurance_inspect';
...
async function verifyTools() {
    ...
    for (const t of filterTests) {
        try {
            ...
            const res = await InsuranceFilter.exec(t.params);
    ...
    for (const t of searchTests) {
        try {
            process.stdout.write(`Testing ${t.name}... `);
            const res = await InsuranceSearch.exec({ keyword: t.keyword, limit: 3 });
            if (res.ok && res.products && res.products.length > 0) {
                ...
                res.products.forEach((p, idx) => {
                    ...
                    if (p.matches?.coverage) console.log(`      Snippet: ${p.matches.coverage.substring(0, 60)}...`);
                    if (p.matches?.extend_info) console.log(`      Extend: ${p.matches.extend_info.substring(0, 60)}...`);
                });
            } else {
    ...
    for (const t of inspectTests) {
        try {
            process.stdout.write(`Testing ${t.name}... `);
            const res = await InsuranceInspect.exec(t.params);
            if (res.ok && res.data) {
                console.log(`[PASS]`);
