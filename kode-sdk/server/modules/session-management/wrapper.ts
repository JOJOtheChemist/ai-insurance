...
import { sessionStorage as originalStorage } from './storage';
...
class WrappedSessionStorage {
  ...
  getAllSessionIds(): string[] {
    return originalStorage.getAllSessionIds();
  }
  ...
  sessionExists(agentId: string): boolean {
    return originalStorage.sessionExists(agentId);
  }
  ...
  getSessionPath(agentId: string): string {
    return originalStorage.getSessionPath(agentId);
  }
  ...
  readMeta(agentId: string): SessionMeta {
    return originalStorage.readMeta(agentId);
  }
  ...
  writeMeta(agentId: string, meta: SessionMeta): void {
    // 1. 原有操作：写入文件
    originalStorage.writeMeta(agentId, meta);
  ...
  readMessages(agentId: string): any[] {
    return originalStorage.readMessages(agentId);
  }
  ...
  deleteSession(agentId: string): boolean {
    // 1. 原有操作：删除文件
    const result = originalStorage.deleteSession(agentId);
  ...
  renameSession(agentId: string, newName: string): boolean {
    // 1. 原有操作：重命名文件
    const result = originalStorage.renameSession(agentId, newName);
  ...
  getSessionStats(agentId: string): {
    ...
    return originalStorage.getSessionStats(agentId);
  }
 ...
 * 导出包装后的存储实例
 */
export const wrappedStorage = new WrappedSessionStorage();
