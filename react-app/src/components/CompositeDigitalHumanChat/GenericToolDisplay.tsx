...
export const GenericToolDisplay: React.FC<GenericToolDisplayProps> = ({ tool }) => {
    ...
    return (
        <div className="mb-2 bg-white rounded-xl border border-gray-100 overflow-hidden w-full max-w-full shadow-sm hover:shadow-md transition-all duration-300">
            <div
                className="flex items-center gap-3 p-2.5 px-3.5 cursor-pointer hover:bg-[#F9FAFB] transition-colors"
                onClick={() => setIsOpen(!isOpen)}
            >
                <div className="shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-[#F0F7FF]">
                    {tool.status === 'running' && (
            ...
            <div className={`transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[500px] border-t border-gray-100 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                <div className="p-3 bg-[#F9FAFB] text-[11px] font-mono text-gray-500 overflow-x-auto space-y-2">
                    {tool.args && (
                        <div className="flex gap-2">
                            <span className="text-gray-400 shrink-0 font-bold uppercase tracking-tighter">输入:</span>
                            <span className="text-blue-600 break-all">{JSON.stringify(tool.args, null, 2)}</span>
                        </div>
                    )}
                    {tool.result && (
                        <div className="flex gap-2 pt-1 border-t border-gray-200">
                            <span className="text-gray-400 shrink-0 font-bold uppercase tracking-tighter">输出:</span>
                            <span className="text-green-600 break-all">{typeof tool.result === 'string' ? tool.result : JSON.stringify(tool.result, null, 2)}</span>
                        </div>
                    )}
                </div>
            </div>
